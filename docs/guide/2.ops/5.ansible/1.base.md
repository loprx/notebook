...
幂等性是 Ansible 的核心概念之一，指的是任务在多次执行后，目标系统的状态保持一致，不会因为重复执行而产生额外的变化。幂等性确保了自动化操作的安全性和可预测性。

Ansible 的大多数内置模块（如 file、copy、yum 等）都支持幂等性。例如：

- 使用 file 模块创建文件夹，如果文件夹已存在，则不会重复创建。
- 使用 copy 模块复制文件，如果文件内容未改变，则不会覆盖。
- 使用 yum 模块安装软件包，如果软件包已安装，则不会重复安装。
